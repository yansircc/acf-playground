<script lang="ts">
  ...
  function buildSlugifyPayload(
    ...
    return entities
      .filter((e) => entityIssueIds.has(e.id) || collectAllFields(allFields(e)).some((f) => fieldIssueIds.has(f.id)))
      .map((e) => ({
        id: e.id,
        name: e.name,
        isTaxonomy: isTaxonomyEntity(e),
        needSlug: entityIssueIds.has(e.id),
        fields: collectAllFields(allFields(e))
  ...
  function applyLLMResult(
    ...
    for (const entity of entities) {
      normalizeFieldScope(allFields(entity), fieldIssueIds, llmFieldMap, overrides);
    }
  ...
  function buildDeterministicOverrides(
    ...
    for (const entity of entities) {
      ...
      normalizeFieldScope(allFields(entity), fieldIssueIds, emptyLLM, overrides);
    }
